...
- **录制流程**：工具栏 `Start Record` 清空录制缓冲并开始采集；`Stop Record` 结束当前录制段；
  `Save Record` 只保存该录制段，不再保存连接以来的全部内存缓存。
- **保存格式**：保存时可选择 `.rttcap` / `.csv` / `.json`。
  `.rttcap` 为 zip 包，包含 `raw.log`（录制段日志文本）、`data.npz`（录制段通道数组）、
  `meta.json`（通道模型、标记、会话信息）；CSV 仅导出录制段通道表；JSON 导出录制段结构化数据。
...
| M4 | 打标 + 分段录制/保存/回放 | 打标、开始/结束录制、按格式保存、离线浏览 |
